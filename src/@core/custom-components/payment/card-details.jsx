...
const CardDetails = ({setCardDetails, cardDetails, handleNextStep}) => {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            name: Yup.string().matches(/^[A-Za-z]+(?:[ '-][A-Za-z]+)*$/, 'Invalid Name').required('Name is required'),
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid style={{ margin: '5px 0px' }} item xs={6}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-evenly' }}>
                                        <Button onClick={() => datePickerRef.current.openCalendar()}><EventIcon style={{ fontSize: '30px' }} /></Button>
                                        <Typography
