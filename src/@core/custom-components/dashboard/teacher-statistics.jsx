...
const TeacherStatistics = ({ statistics, loading }) => {
            ...
            <Grid spacing={10} container>
                {
                    !!handleGetStaticsArray(statistics).length && !loading ? handleGetStaticsArray(statistics)?.map(({ title, value }, _) => (
                        <Grid style={{ display: 'flex', justifyContent: 'center' }} key={_} item md={6} sm={12} xs={12}>
                            <StatisticsCard strokeWidth={6} value={value}>
